...
function loadSavedSnapshot(): SyncStateSnapshot | null {
  try {
    const raw = localStorage.getItem('kgb_saved_room')
    if (!raw) return null
...
function clearSavedSnapshot() {
  localStorage.removeItem('kgb_saved_room')
}

function persistSnapshot(snapshot: SyncStateSnapshot) {
  try {
    localStorage.setItem('kgb_saved_room', JSON.stringify(snapshot))
  } catch {
...
function App() {
  ...
  const [savedSnapshot, setSavedSnapshot] = useState<SyncStateSnapshot | null>(loadSavedSnapshot)

  const metroSettingsRef = useRef<HTMLDivElement>(null)
  // Always-current snapshot for beforeunload — avoids stale closure
  const latestSnapshotRef = useRef<SyncStateSnapshot | null>(null)
  ...
  // Autosave room state — fires on meaningful changes, not on per-step playback ticks
  useEffect(() => {
    const snapshot: SyncStateSnapshot = {
      bpm,
      isPlaying,
      currentStep: 0,
      activePatternIndex: machineState.activePatternIndex,
      swing: machineState.swing,
      patternBank: drumMachine.getPatternBank() as SyncStateSnapshot['patternBank'],
      metronomeEnabled: metronomeState.enabled,
      timeSignature: metronomeState.timeSignature,
      chain: machineState.chain,
      strongBeatIndex: metronomeState.strongBeatIndex,
      syncOnly,
      playStartAt: null,
    }
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
    ...
    bpm,
    machineState.activePatternIndex,
    machineState.swing,
    machineState.stepCount,
    machineState.chain,
    machineState.pattern,
    machineState.velocity,
    metronomeState.enabled,
    metronomeState.timeSignature,
    metronomeState.strongBeatIndex,
    syncOnly,
    isPlaying,
  ])

  // Crash protection — save snapshot synchronously on page close/crash
  useEffect(() => {
  ...
  useEffect(
    () =>
      roomSyncClient.subscribeRtt(({ socketId, rtt }) => {
        ...
        // Track own RTT for clock sync drift correction
        if (socketId === roomSyncClient.getState().socketId) {
          ownRttRef.current = rtt
        }
      }),
    [],
  )

  // NTP-like clock sync — correct Tone.Transport drift toward server timeline
  useEffect(
    () =>
      roomSyncClient.subscribeClockGrid((event) => {
        clockSync.update(event, ownRttRef.current)
      }),
    [],
  )
  ...
  useEffect(
    () =>
      roomSyncClient.subscribeSyncEvents(async (event) => {
        ...
        if (event.type === 'strong_beat_change') {
          metronome.setStrongBeatIndex(event.payload.strongBeatIndex)
          return
        }

        if (event.type === 'sync_only_toggle') {
          setSyncOnly(event.payload.enabled)
          metronome.setSoundEnabled(!event.payload.enabled)
          return
        }
  ...
  const handleSyncOnlyToggle = async () => {
    if (roomState.roomId && !roomState.isHost) return
    const next = !syncOnly
    setSyncOnly(next)
    metronome.setSoundEnabled(!next)
    await emitSyncEvent({ type: 'sync_only_toggle', payload: { enabled: next } })
  }

  const handleStrongBeatChange = async (index: number) => {
    if (roomState.roomId && !roomState.isHost) return
    metronome.setStrongBeatIndex(index)
    await emitSyncEvent({ type: 'strong_beat_change', payload: { strongBeatIndex: index } })
  }
  ...
  const handleOpenSavedRoom = async () => {
    if (!savedSnapshot) return
    await applySyncSnapshot(savedSnapshot)
    clearSavedSnapshot()
    setSavedSnapshot(null)
  }

  const handleDiscardSavedRoom = () => {
    clearSavedSnapshot()
    setSavedSnapshot(null)
  }
  ...
  const applySyncSnapshot = async (snapshot: SyncStateSnapshot | null) => {
    ...
    metronome.setStrongBeatIndex(snapshot.strongBeatIndex ?? 0)
    const snapshotSyncOnly = snapshot.syncOnly ?? false
    setSyncOnly(snapshotSyncOnly)
    metronome.setSoundEnabled(!snapshotSyncOnly)
    clockSync.setBaseBpm(snapshot.bpm)
    ...
    <main className={[
      ...
      {!inRoom ? (
        <section className="lobby" aria-label="Join or host a room">
          ...
          {savedSnapshot && (
            <div className="lobby-saved-room">
              <p className="eyebrow">Saved session</p>
              <div className="saved-room-card">
                <div className="saved-room-info">
                  <span className="saved-room-detail">{savedSnapshot.bpm} BPM</span>
                  <span className="saved-room-detail">
                    {savedSnapshot.timeSignature.beats}/{savedSnapshot.timeSignature.division}
                  </span>
                  <span className="saved-room-detail">
                    {savedSnapshot.patternBank.filter((s) =>
                      Object.values(s.pattern).some((track) => track.some(Boolean))
                    ).length} patterns
                  </span>
                </div>
                <div className="saved-room-actions">
                  <button
                    type="button"
                    className="primary-action"
                    onClick={() => void handleOpenSavedRoom()}
                  >
                    Resume
                  </button>
                  <button
                    ...
                    onClick={handleDiscardSavedRoom}
                  >
                    Discard
                  </button>
                </div>
          ...
          {showMetroSettings && (
            <div className="metro-settings-popover" role="dialog" aria-label="Metronome settings">
              ...
              <label className="metro-settings-row">
                ...
                <select
                  aria-label="Strong beat"
                  className="time-sig-select"
                  disabled={inRoom && !roomState.isHost}
                  value={metronomeState.strongBeatIndex}
                  onChange={(e) => { void handleStrongBeatChange(Number(e.target.value)) }}
                >
                  {Array.from({ length: metronomeState.timeSignature.beats }, (_, i) => (
              ...
              <label className="metro-settings-row metro-settings-check">
                <input
                  type="checkbox"
                  checked={syncOnly}
                  onChange={() => { void handleSyncOnlyToggle() }}
                  disabled={inRoom && !roomState.isHost}
                />
                <span>Sync only</span>
              </label>
