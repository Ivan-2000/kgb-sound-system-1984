...
| **N2** | — | ✅§5.1 ✅§5.2 ✅§5.3 ✅§5.5 ✅§5.6 ✅§5.7 ✅§5.8(local) ✅§5.9 ✅§5.10 ✅§5.15 | §5.16 ✅§5.17(clips) §5.18 |
...
**N2 accepted/deferred (2026-07-22):** §5.8 закрыт для локальной записи (гард на смену BPM);
кросс-юзерный кейс (удалённый хост меняет темп, пока ты пишешь) отложен — нужен серверный
трекинг record-state. §5.16 (beat-индекс при смене BPM) и §5.18 (MIDI vs аудио на playback в
другом темпе) — MED/LOW, отложены. §5.7 закрыт перечитыванием позиции транспорта перед
`recorder.start` (App.tsx:1294); §5.1/§5.2/§5.10 были уже сделаны в record-disarm.
