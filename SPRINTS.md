...
4. **Отметить готовым в ДВУХ местах:** (а) `[ ]`→`[x]` в `TASKS.md` (фичи) или статус
   в `AUDIT.md` (§-пункт); (б) поставить ✅ напротив ID в «Coverage ledger» этого файла.
   Так и фича, и находка имеют единый видимый журнал готовности.
