...
### E5 — Opus worker-thread (§9.A.1 / §1.5) — CRIT RT-safety
```
Репо A:\KGB, main. Прочитай SPRINTS.md (DoD + ledger E5), AUDIT.md §9.A.1 и §1.5.
Зона — addon.cc (C++ only, ~1600 строк). Используй Opus (не Sonnet) — решение
затрагивает 3+ слоя нативного кода с нетривиальной RT-синхронизацией.

Проблема: Opus encode/decode сейчас выполняется прямо в PortAudio RT callback.
malloc() внутри callback → периодические xrun (щелчки у всех участников) при нагрузке.

Что нужно сделать:
1. Добавить отдельный `std::thread` (OpusThread) в addon.cc — один на процесс.
2. SPSC-кольцо RT→OpusThread: сырые PCM float32 сэмплы (накопленный Opus-фрейм).
   RT callback только пишет в кольцо и выходит. Без malloc, без блокировок.
3. OpusThread кодирует → вызывает broadcastOpusPacket (уже на non-RT потоке).
4. Для декодирования: входящие Opus-пакеты (PushInboundOpus) уже на JS-потоке —
   декодирование тоже перенести в OpusThread, результат класть в PeerRing.
5. Убедиться что PeerRing (уже SPSC) корректно работает с новым продюсером.
6. Старый malloc/decode-путь в PaCallback удалить.
...
| **E4** | ✅I1 ✅I3 ✅T3 ✅mixdown WAV/MP3 ✅project-export | §1.5 §9.A.1 ✅§9.A.2 §9.A.3 ✅§9.A.4 ✅§9.A.5 ✅§9.B.2 ✅§9.D.1 ✅§9.D.2 ✅§9.D.3 / §1.5 §9.A.1 → closed in E5; §9.A.3 deferred | §9.A.6–12 §9.D.4–10 |
| **E5** | — | ✅§9.A.1 ✅§1.5 | — |
| **N1** | — | ✅§3.1 ✅§3.2 ✅§3.3 ✅§3.4 ✅§3.5 ✅§8.B.1 | ✅§3.7 ✅§8.B.2 ✅§8.B.5 / §3.6 §8.B.3 §8.B.4 §8.B.6 (accepted/deferred, см. ниже) |
| **N2** | — | ✅§5.1 ✅§5.2 ✅§5.3 ✅§5.5 ✅§5.6 ✅§5.7 ✅§5.8 ✅§5.9 ✅§5.10 ✅§5.15 | ✅§5.16 ✅§5.17(clips) ✅§5.18 |
| **N3** | — | ✅§8.C.1 §8.C.2→N4 ✅§8.C.3 ✅§5.4 | §8.C.4–6 ✅§5.11 ✅§5.12 ✅§5.13 ✅§5.14 ✅§5.19 |
...
**N1 accepted/deferred (LOW, 2026-07-21):** §3.6 — `cors:'*'` (десктоп-модель, изоляция комнат
+ rate-limit) и reconnect-bypass лимитера приняты как есть; §8.B.3 — mic/cam опоздавшего по
дефолту `true` (per-user transit) отложено в N2 (там живёт per-user snapshot-sync); §8.B.4 —
host-reassign гонка самовосстанавливается следующим `room:host`; §8.B.6 — дубль `ICE_SERVERS`
(`peerManager.ts` / `nativeRtcManager.ts`) — косметика, вынос в общий модуль кросс-трековый, отложен.

**N2 закрыт полностью (2026-07-22).** §5.8 — теперь и кросс-юзер: сервер трекает record-state
(`record:set` → `recordingSockets`), `bpm_change` отклоняется `RECORDING_IN_PROGRESS` пока
кто-либо в комнате пишет; клиент откатывает оптимистичный BPM при отказе. §5.16 — доля из
музыкальных тиков (`getTicksAtTime`/PPQ), не из секунд/beatSec — верно при смене темпа.
§5.18 — MIDI-ноты планируются в тиках (`${ticks}i`), жёстко на сетке, рефлоу с темпом.
§5.7 — перечитывание позиции транспорта перед `recorder.start`; §5.1/§5.2/§5.10 — в record-disarm.

**N3 progress (2026-07-22):** ✅§8.C.1 (HMR-дубль подписок), ✅§5.4 (setState из аудио-колбэка →
Tone.Draw), ✅§5.12 (stream-close финализирует запись, не теряет), ✅§5.13 (double-record
предотвращён оркестрацией toggleArmed + warning в recorder), ✅§5.14 (уже чистилось в
removeClip: blob+буфер). **§8.C.2 → перенесён в N4:** это не баг, а отсутствующая фича —
MIDI-клипы вообще не синкаются (нужны согласование id, рассылка при создании/правке,
троттлинг редактора). Осталось в N3: §8.C.4–6 (MED/LOW) + **декомпозиция App.tsx** (крупный рефактор — отдельная свежая сессия).
Добито 2026-07-22: ✅§8.C.3 (unmount-cleanup live-таймера + явный stop при выходе из комнаты),
✅§5.11 (durSec каждый тик, тяжёлый peaks-массив троттлится ~600мс), ✅§5.19 (setState убран из
рендера редактора нот — просто `return null`, стейл-id безвреден).
