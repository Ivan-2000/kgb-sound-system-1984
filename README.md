...
**UI — реальный нода-граф:** каждый инструмент (миксер, метроном, драм-машина, таймлайн, пиано-рол, math-ноды…) — отдельная нода с типизированными входами/выходами; ноды обмениваются данными; сторонние разработчики пишут свои ноды по контракту и подключают их. Два вида одного графа: **Panels** (ноды как панели без связей — для простых сценариев) и **Canvas** (холст с кабелями — для продвинутых). Стратегия и контракт — в `AGENTS.md`, `TASKS_UI.md`, `client/src/graph/types.ts`.
