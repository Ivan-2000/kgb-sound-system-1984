...
class RoomManager {
  ...
  getSyncState(roomId) {
    ...
    return {
      ...
      strongBeatIndex: s.strongBeatIndex,
      syncOnly: s.syncOnly,
      playStartAt: s.playStartAt,
    }
  }
  ...
  applySyncEvent(roomId, event) {
    ...
    if (event.type === 'transport_play') {
      s.isPlaying = true; s.currentStep = event.payload.step; s.playStartAt = Date.now(); return
    }

    if (event.type === 'transport_stop') {
      s.isPlaying = false; s.currentStep = event.payload.step; s.playStartAt = null; return
    }
    ...
    if (event.type === 'strong_beat_change') {
      s.strongBeatIndex = event.payload.strongBeatIndex; return
    }

    if (event.type === 'sync_only_toggle') {
      s.syncOnly = event.payload.enabled; return
    }
  }
}
