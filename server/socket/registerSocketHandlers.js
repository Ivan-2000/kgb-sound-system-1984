...
function registerSocketHandlers(io, roomManager) {
  ...
  const clockGridIntervals = new Map()
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('room:event', (rawEvent, ack) => {
      ...
      const hostOnlyTypes = new Set(['transport_play', 'transport_stop', 'bpm_change', 'step_count_change', 'time_signature_change', 'metronome_toggle', 'swing_change', 'pattern_switch', 'chain_set', 'strong_beat_change', 'sync_only_toggle'])
      ...
      if (parsed.data.type === 'transport_play') {
        if (clockGridIntervals.has(roomId)) {
          clearInterval(clockGridIntervals.get(roomId))
        }
        const intervalId = setInterval(() => {
          const currentRoom = roomManager.getRoom(roomId)
          if (!currentRoom || !currentRoom.syncState.isPlaying) {
            clearInterval(clockGridIntervals.get(roomId))
            clockGridIntervals.delete(roomId)
            return
          }
          io.to(roomId).emit('clock_grid', {
            serverTime: Date.now(),
            playStartAt: currentRoom.syncState.playStartAt,
            bpm: currentRoom.syncState.bpm,
          })
        }, 2000)
        clockGridIntervals.set(roomId, intervalId)
        // Emit immediately so clients get a reference point right away
        io.to(roomId).emit('clock_grid', {
          serverTime: Date.now(),
          playStartAt: room.syncState.playStartAt,
          bpm: room.syncState.bpm,
        })
      }

      if (parsed.data.type === 'transport_stop') {
        if (clockGridIntervals.has(roomId)) {
          clearInterval(clockGridIntervals.get(roomId))
          clockGridIntervals.delete(roomId)
        }
