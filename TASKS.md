...
```
B1. Phase 1 signaling — ✅ ЗАВЕРШЁН
    Ping/RTT, пароль на комнату, лимит участников, история комнат

B2. Phase 4 — drum machine и метроном — ✅ ЗАВЕРШЁН (~95%, остаток ждёт Phase 3)
    Velocity, swing, multiple patterns, clock sync, NTP drift correction

B3. Phase 5 UI — В РАБОТЕ (~85%)
    Тулбар, чат, права хоста, панель настроек, Host API UI, buffer size UI,
    автосохранение, защита от крэша, «открыть сохранённую комнату»
    Осталось: нагрузочное тестирование, обработка отключения/переподключения устройства
```
...
- [x] Выбор сильной доли (strong beat) — UI в настройках метронома, синхронизирован по комнате
- [x] Режим только sync (звук метронома отключён, сигнал для DAW сохранён) — синхронизирован по комнате
...
- [x] NTP-подобная синхронизация тактовой сетки (clock sync) — сервер отправляет clock_grid каждые 2с
- [x] Коррекция дрейфа (drift correction) — плавная коррекция BPM в clockSync.ts

### Критерий готовности

- [x] Драм-машина работает и синхронизируется
- [x] Все участники слышат метроном синхронно (NTP clock sync + drift correction)
...
- [x] Выбор Host API для устройства: авто / ASIO / WASAPI Exclusive / WASAPI Shared / DirectSound
- [x] Настройка размера буфера (buffer size)
- [x] Выбор формата записи (WAV / MP3)
- [x] Кнопка «Переинициализировать устройство» без перезапуска приложения

**Стабильность:**
- [x] Автосохранение состояния комнаты
- [x] Защита от потери данных при крэше
...
| К | Автосохранение состояния комнаты | [x] Готово |
...
| Phase 4 — Метроном и драм-машина | ~95% *(экспорт MIDI в Phase 3)* |
| Phase 5 — UI и полировка | ~85% *(нагрузочное тестирование и переподключение устройства — последнее)* |

---

*KGB Sound TASKS.md v1.8 — основан на kgb_sound_roadmap.md v1.1. Оригинал сохранён в TASKS_v1_original.md.*
