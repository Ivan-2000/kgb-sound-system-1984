...
- [ ] Экран выбора аудиоустройства (доступен при запуске и из настроек)
- [ ] Список устройств: название, доступные Host API, число каналов
...
- [ ] Кнопка «Переинициализировать устройство» без перезапуска приложения
...
| Фаза | Прогресс |
|---|---|
| Phase 1 — Сеть и комнаты | ~50% *(нативный аудиопуть не начат)* |
| Phase 2 — Миксер и запись | ~25% *(ждёт A3)* |
| Phase 3 — Монтажный стол и MIDI | 0% *(ждёт Phase 2)* |
| Phase 4 — Метроном и драм-машина | ~40% *(Поток B2, можно делать сейчас)* |
| Phase 5 — UI и полировка | ~50% *(Поток B3, можно делать сейчас)* |

---

*KGB Sound TASKS.md v1.4 — основан на kgb_sound_roadmap.md v1.1. Оригинал сохранён в TASKS_v1_original.md.*
