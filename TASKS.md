...
- [ ] **T1 — Snap to grid** — привязка клипов и краёв к BPM-сетке (доли / такты), переключаемая
- [ ] **T2 — Реальная запись на armed-дорожку** — нативный захват (PortAudio input → buffer) → конверт в WAV → клип на armed-аудиодорожке; пока идёт конверт — прокси-клип-плейсхолдер
- [ ] **T3 — MP3-кодирование в фоне** — ffmpeg-wasm worker, не блокирует UI
- [ ] **T4 — Синк клипов между участниками** — событие `clip_add/clip_remove/clip_move/clip_resize` через `syncProtocol`, синхронизация позиций/длин; синк файла отдельным каналом (бинарный chunk-стрим), прокси→реальный при приходе
...
- [ ] Записанный аудио-клип появляется на таймлайне у всех участников (синк T4)
